...
def test_get_a_post():
    app_logger.info("Starting get a post test")
    post_id = 1
    response = api.get_a_post(post_id)
    app_logger.info(f"Response for post {post_id}: {response.json()}")
...
def test_get_all_posts():
    app_logger.info("Starting get all posts test")
    response = api.get_all_posts()
    app_logger.info(f"Response for all posts: {response.json()}")
...
def test_create_post(post_data):
    app_logger.info("Starting create post test")
    response = api.create_post(
        post_data["title"],
        post_data["body"],
        post_data["userId"]
        )
    app_logger.info(f"Response for creating post: {response.json()}")
